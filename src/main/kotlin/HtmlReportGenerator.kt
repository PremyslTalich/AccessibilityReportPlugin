...
<style>
  * { box-sizing: border-box; margin: 0; padding: 0; }
  body { font-family: 'Segoe UI', Arial, sans-serif; background: #1e1e1e; color: #d4d4d4; height: 100vh; display: grid; grid-template-columns: 50% 50%; grid-template-rows: auto 1fr auto; grid-template-areas: 'header screenshot' 'tree screenshot' 'props screenshot'; }
  h1 { grid-area: header; font-size: 1.1rem; padding: 10px 16px; background: #2d2d2d; border-bottom: 1px solid #3c3c3c; color: #cccccc; display: flex; align-items: center; gap: 8px; }
  h1 svg { width: 24px; height: 24px; flex-shrink: 0; }
  #tree-container { grid-area: tree; overflow: auto; padding: 8px; border-right: 1px solid #3c3c3c; }
  #props-container { grid-area: props; height: 220px; border-top: 1px solid #3c3c3c; border-right: 1px solid #3c3c3c; overflow: auto; }
  #right { grid-area: screenshot; overflow: auto; display: flex; align-items: flex-start; justify-content: center; background: #252526; position: relative; }
...
function buildTree(nodeList, parentUl, depth) {
  nodeList.forEach(n => {
    ...
    div.addEventListener('click', () => selectNode(n, div));
